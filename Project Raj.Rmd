...
Djokovic = filter(Tennis, str_detect(winner_name,"Novak Djokovic" )|str_detect(loser_name, "Novak Djokovic"))
```

```{r data aggregation}
tennis10$year <- substring(tennis10$tourney_date, 1, 4)
tennis10$qtr <- ceiling(as.numeric(substring(tennis10$tourney_date, 5, 6))/3)
tennis10$grandslam <- ifelse(tennis10$tourney_level == "G", "grandslam", "non-grandslam")


total_wins <- tennis10 %>% group_by(winner_name, year, qtr, surface, grandslam)%>%
  dplyr::summarize(total_wins = n(),
            total_win_rounds = mean(round, na.rm = TRUE),
            total_win_aces = sum(w_ace,  na.rm = TRUE),
            total_win_df = sum(w_df,  na.rm = TRUE),
            total_win_svpt = mean(w_svpt, na.rm = TRUE),
            total_win_1stIn = mean(w_1stIn, na.rm = TRUE),
            total_win_1stWon = mean(w_1stWon, na.rm = TRUE),
            total_win_2ndWon = mean(w_2ndWon, na.rm = TRUE),
            total_win_bpSaved = sum(w_bpSaved, na.rm = TRUE),
            total_win_bpFaced = sum(w_bpFaced, na.rm = TRUE))

total_loses <- tennis10 %>% group_by(loser_name, year, qtr, surface, grandslam)%>%
  dplyr::summarize(total_loses = n(),
            total_lose_rounds = mean(round, na.rm = TRUE),
            total_lose_aces = sum(l_ace, na.rm = TRUE),
            total_lose_df = sum(l_df, na.rm = TRUE),
            total_lose_svpt = mean(l_svpt, na.rm = TRUE),
            total_lose_1stIn = mean(l_1stIn, na.rm = TRUE),
            total_lose_1stWon = mean(l_1stWon, na.rm = TRUE),
            total_lose_2ndWon = mean(l_2ndWon, na.rm = TRUE),
            total_lose_bpSaved = sum(l_bpSaved, na.rm = TRUE),
            total_lose_bpFaced = sum(l_bpFaced, na.rm = TRUE))
...
```{r}
total_w <- tennis10 %>% group_by(winner_name, year, qtr)%>%
  dplyr::summarize(total_wins = n(),
...
total_l <- tennis10 %>% group_by(loser_name, year, qtr)%>%
  dplyr::summarize(total_loses = n(),
...
total_w$concat <- paste(total_w$winner_name, total_w$year, total_w$qtr, sep="_")
total_l$concat <- paste(total_l$loser_name, total_l$year,  total_l$qtr, sep="_")
