...
```{r}
tennis10$tourney_date = as.Date(tennis10$tourney_date)
line1 = ggplot(tennis10,aes(tourney_date, w_ace))
line1 +
  stat_summary(fun.y = mean, ##adds the points
               geom = "point")
barplot(tourney_date~tourney_id, data=tennis10)
hist(tennis10$tourney_date, breaks=500)
```
```{r }
library(Hmisc)
graphdata = read.csv("05_data.csv")
bar1 = ggplot(graphdata, aes(biz_focus, time.2, fill = internet))
bar1 +
  ...
  stat_summary(fun.data = mean_cl_normal,
               ...
               width = .2) +
  ggtitle("2nd measurement Sales score across \ndifferent Business focus & Internet use") +
  xlab("Business Marketing Focus") +
  scale_x_discrete(labels=c("forward" = "Forward", "traditional" = "Traditional")) +
  ylab("Sales score @ t2") + 
 # cleanup +
  scale_fill_manual(name = "Internet use", 
                    labels = c("Low", "Medium", "High"),
                    values = c("Gray", "Gray40","Gray20")) +
  theme_classic()
